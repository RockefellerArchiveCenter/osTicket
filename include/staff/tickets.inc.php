...
<div id='basic_search'>
    ...
    <input class="btn btn-primary" type="submit" name="basic_search" value="Search" onClick="ga('send', 'event', 'Search', 'Basic', '<?php echo $status?>');">
...
<?php csrf_token(); ?>
 ...
 <table class="table table-striped" border="0" cellspacing="1" cellpadding="2" width="100%">
     ...
     <tbody>
        ...
        if($res && ($num=count($results))):
            ...
            <tr id="<?php echo $row['ticket_id']; ?>">
                ...
                <td nowrap><?php echo Format::db_datetime($row['created']); ?></td>
    ...
    if($num>0){ //if we actually had any tickets returned.
        ...
        echo '<a class="btn btn-primary pull-right export-csv" href="?a=export&h='
            .$hash.'&status='.$_REQUEST['status'] .'">Export</a>';
    ?>
        <?php
    ...
    <form action="tickets.php" method="post" id="search" name="search">
        ...
        <fieldset>
            ...
            <select class="form-control" id="status" name="status">
                <option value="" onClick="ga('send', 'event', 'Search', 'Advanced', 'Status Any');">&mdash; Any Status &mdash;</option>
                <option value="open">Open</option>
                <?php
                if(!$cfg->showAnsweredTickets()) {?>
                <option value="answered" onClick="ga('send', 'event', 'Search', 'Advanced', 'Status Answered');">Answered</option>
                <?php
                } ?>
                <option value="overdue" onClick="ga('send', 'event', 'Search', 'Advanced', 'Status Overdue');">Overdue</option>
                <option value="closed" onClick="ga('send', 'event', 'Search', 'Advanced', 'Status Closed');">Closed</option>
            </select>
            <label for="deptId">Dept:</label>
            <select class="form-control" id="deptId" name="deptId">
                <option value="" onClick="ga('send', 'event', 'Search', 'Advanced', 'Department All');">&mdash; All Departments &mdash;</option>
                <?php
                if(($mydepts = $thisstaff->getDepts()) && ($depts=Dept::getDepartments())) {
                    foreach($depts as $id =>$name) {
                        if(!in_array($id, $mydepts)) continue;
                        echo sprintf('<option value="%d" onClick="ga(\'send\', \'event\', \'Search\', \'Advanced\', \'Department %s\');">%s</option>', $id, $name, $name);
                    }
                }
                ?>
            </select>
        ...
        <fieldset class="owner">
            ...
            <select class="form-control" id="assignee" name="assignee">
                <option value="" onClick="ga('send', 'event', 'Search', 'Advanced', 'Assigned to Anyone');">&mdash; Anyone &mdash;</option>
                <option value="0" onClick="ga('send', 'event', 'Search', 'Advanced', 'Assigned to Unassigned');">&mdash; Unassigned &mdash;</option>
                ...
                if(($users=Staff::getStaffMembers())) {
                    ...
                    foreach($users as $id => $name) {
                        $k="s$id";
                        echo sprintf('<option value="%s" onClick="ga(\'send\', \'event\', \'Search\', \'Advanced\', \'Assigned to %s\');">%s</option>', $k, $name, $name);
                    }
                    echo '</OPTGROUP>';
                }
                ...
                if(($teams=Team::getTeams())) {
                    ...
                    foreach($teams as $id => $name) {
                        $k="t$id";
                        echo sprintf('<option value="%s" onClick="ga(\'send\', \'event\', \'Search\', \'Advanced\', \'Assigned to team %s\');">%s</option>', $k, $name, $name);
                    }
                    echo '</OPTGROUP>';
                }
                ?>
            ...
            <select class="form-control" id="staffId" name="staffId">
                <option value="0" onClick="ga('send', 'event', 'Search', 'Advanced', 'Closed by Anyone');">&mdash; Anyone &mdash;</option>
                <option value="<?php echo $thisstaff->getId(); ?>" onClick="ga('send', 'event', 'Search', 'Advanced', 'Closed by Me');">Me</option>
                <?php
                if(($users=Staff::getStaffMembers())) {
                    foreach($users as $id => $name)
                        echo sprintf('<option value="%d" onClick="ga(\'send\', \'event\', \'Search\', \'Advanced\', \'Closed by %s\');">%s</option>', $id, $name, $name);
                }
                ?>
            </select>
        </fieldset>
